...
#'
#' @examples location_test(Genotype,Data$Y,
#'                         Data$Sex,
#'                         Covariate=Data[,"Age"],
#'                         missing_cutoff=0.15,
#'                         MAF_Cutoff=NULL,
...
location_test <- function(Genotype,Y,Sex,
  ...
  res <- t(apply(Genotype, 2, function(snp)
  {
    ...
    names(res) <- c('PQXcat.test_fisher','PQZmax.test_fisher','PQXcat.p_fisher','PQZmax.p_fisher')
    return(res)
  }))
  ...
  colnames(Tstat) <- c('SNP','PQXcat','PQZmax')
  pvalue <- data.frame(SNP=SNPnames,res[,3:4,drop=F],row.names = NULL)
  colnames(pvalue) <- c('SNP','PQXcat','PQZmax')
