###########################################################
# mean-based method: MTchen
###########################################################

#' @title mean-based method: MTchen, including a dominant effect of heterozygous (Aa) females
#' @description
#' A function to obtain the p value and the test statistic of MTchen for testing the mean difference of the trait value across genotypes.
#'
#'
...
#'
#' @return The p value and test statistic of MTchen.
#' @export
#'
#' @examples
MTchen_test <- function(Genotype,Y,Sex,Covariate=NULL,
                        missing_cutoff=0.15,
                        MAF_Cutoff=NULL,
                        MGC_Cutoff=20,
                        kins=kins){
  ...
  res <- t(apply(Genotype, 2, function(snp){
    ...
    names(res) <- c('MTchen.test','MTchen.p')
    return(res)
  }))
