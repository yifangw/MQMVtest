###########################################################
# mean-based method:MTplinkw
###########################################################


#' @title mean-based method:MTplinkw
#' @description
#' A function to obtain the p value and the test statistic of MTplinkw for testing the mean difference of the trait value across genotypes.
#'
...
MTplinkw_test <- function(Genotype,Y,Sex,
  ...
  res <- t(apply(Genotype, 2, function(snp){
    ...
    names(res) <- c('MTplinkw.test','MTplinkw.p')
    return(res)
  }))
  ...
  colnames(Tstat) <- c('SNP','MTplinkw')
  pvalue <- data.frame(SNP=SNPnames,res[,2,drop=F],row.names = NULL)
  colnames(pvalue) <- c('SNP','MTplinkw')
